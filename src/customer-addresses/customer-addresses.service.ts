...
export class CustomerAddressesService {
    ...
    async create(dto: CreateCustomerAddressDto): Promise<CustomerAddress> {
        ...
        if (dto.is_default) {
            ...
            await this.repo.update({ customer_id: dto.customer_id }, { is_default: false });
        }
    ...
    async update(id: string, dto: UpdateCustomerAddressDto): Promise<CustomerAddress | null> {
        if (dto.is_default) {
            const existing = await this.repo.findOne({ where: { id } });
            if (existing) {
                await this.repo.update({ customer_id: existing.customer_id }, { is_default: false });
            }
        }
