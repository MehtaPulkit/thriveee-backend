...
export class SupabaseSyncService {
    ...
    private async handleUserCreated(user: any) {
        ...
        if (role === 'provider') {
            const { error } = await this.supabase.from('providers').insert([{ profile_id: id }]);
            if (error) this.logger.error('Error creating provider record:', error.message);
        } else {
            const { error } = await this.supabase.from('customers').insert([{ profile_id: id }]);
            if (error) this.logger.error('Error creating customer record:', error.message);
        }
